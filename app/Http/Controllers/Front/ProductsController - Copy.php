      ...
      if ($request->isMethod('post')) {
        if ($request->qty <= 1 && $request->button_type == 'sub') {
          ...
          if (!empty($cart_list)) {
            ...
            $result['qty'] = $qty;
            $result['Maintotal'] = $result['shippingamount'] + $result['total'] + $result['tax_amount'];
 ...
 */
  public function cart_detail()
  {
    ...
    return view('front.products.cart_detail',["cart_list" => $cart_list,"addressesArr" => $addressesArr,"shipping_taxes" => $shipping_taxes]);
  }
 ...
 */
  public function shopping_cart_step(Request $request)
  {
    ...
    if (!empty($cart_itemslist[0])) {
      ...
      $maintotal = $total + $shippingamount + $tax_amount;
    }
